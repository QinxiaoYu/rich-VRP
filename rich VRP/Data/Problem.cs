...
=======
	...
	{   //数据集名称
		...
		public static Dictionary<int, AbsNode> AllNodesDict { get; set; }
		...
		public static void setAllNodesDict()
		{
			AllNodesDict = new Dictionary<int, AbsNode>();
			foreach (var nodeinfo in AllNodes)
			{
				AbsNode anode;
				if (nodeinfo.Id<=1000)
				{
					anode = new Customer(nodeinfo);
				}
				else if(nodeinfo.Id>1000)
				{
					anode = new Station(nodeinfo);
				}
				else{
					anode = new Depot(nodeinfo);
				}

				AllNodesDict.Add(nodeinfo.Id,anode);
