...
public class BreakTwoRoute
{
    ...
    public Solution BreakTwoRoute(Solution solution, int break_strategy)
    {
        ...
        for (int i = 0; i < solution.fleet.VehFleet.Count; i++)
        {
            ...
            if (break_strategy == 1)
            {
                ...
                if (nxt_dt > old_r.RouteList[pos_longest_arc].Info.DueDate)
                {
                    continue;
                }
                Route new_r2 = new Route(new_veh, nxt_dt);
                int tmp_count = 1;
                for (int j = pos_longest_arc; j < old_r.RouteList.Count - 1; j++)
                {
                    new_r2.InsertNode(old_r.RouteList[j], tmp_count);
                    tmp_count++;
                }
            ...
            if (break_strategy  == 2)
            {
                ...
                new_veh.VehRouteList.Clear();
                int tmp_count = 1;
                double tmp_battery = old_r.AssignedVehType.MaxRange;
                ...
                while (tmp_count<=num_cus)
                {
                    ...
                    double nxt_charge = new_r.RouteList[nxt_pos-1].TravelDistance(old_r.RouteList[tmp_count]);
                    if (tmp_battery-nxt_charge>0)
                    {
                        new_r.InsertNode(old_r.RouteList[i], nxt_pos);
                        tmp_count++;
                    ...
                    else
                    {
                        ...
                        if (nxt_dt>old_r.RouteList[tmp_count].Info.DueDate)
                        {
                            isSucc = false;
                            break;
                        }
